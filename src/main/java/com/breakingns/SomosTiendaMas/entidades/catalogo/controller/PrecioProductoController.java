...
public class PrecioProductoController {
    ...
    // Crear precio
    @PostMapping("/precios")
    public ResponseEntity<PrecioProductoCentralizadoResponseDTO> crear(@Valid @RequestBody PrecioProductoCrearDTO dto,
                                                           UriComponentsBuilder uriBuilder) {
        PrecioProductoCentralizadoResponseDTO created = service.crear(dto);
    ...
    public ResponseEntity<PrecioProductoCentralizadoResponseDTO> actualizar(@PathVariable Long id,
                                                                @Valid @RequestBody PrecioProductoActualizarDTO dto) {
        PrecioProductoCentralizadoResponseDTO updated = service.actualizar(id, dto);
        return ResponseEntity.ok(updated);
    }

    // Obtener por id
    @GetMapping("/precios/{id}")
    public ResponseEntity<PrecioProductoCentralizadoResponseDTO> obtenerPorId(@PathVariable Long id) {
    ...
    public ResponseEntity<PrecioProductoUIResumenDTO> obtenerVigenteUI(@PathVariable Long productoId) {
        PrecioProductoCentralizadoResponseDTO dto = service.obtenerVigentePorProductoId(productoId);
    ...
    public ResponseEntity<List<PrecioProductoCentralizadoResponseDTO>> listarPorProducto(@PathVariable Long productoId) {
        return ResponseEntity.ok(service.listarPorProductoId(productoId));
    }

    // Listar precios activos (no eliminados)
    @GetMapping("/precios")
    public ResponseEntity<List<PrecioProductoCentralizadoResponseDTO>> listarActivas() {
    ...
    public ResponseEntity<List<PrecioProductoCentralizadoResponseDTO>> buscarVigentesEnFecha(
            @PathVariable Long productoId,
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime fecha) {
        List<PrecioProductoCentralizadoResponseDTO> list = service.buscarVigentesPorProductoIdEnFecha(productoId, fecha);
